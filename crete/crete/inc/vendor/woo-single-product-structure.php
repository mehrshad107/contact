...
if( !function_exists('crete_wc_get_gallery_image_html') ) {
  // Copied and modified from woocommerce plugin and wc_get_gallery_image_html helper function.
  function crete_wc_get_gallery_image_html( $attachment_id, $main_image = false, $size = 'woocommerce_single' ) {
...
add_action('pivoo_woo_sale_hook', 'pivoo_woo_sale_functions', 7);
  ...
  function crete_woo_inventory_functions() {
      global $post, $product;
      if ( $product->get_stock_quantity() ) { // if manage stock is enabled 
if ( number_format($product->get_stock_quantity(),0,'','') < 10 ) { // if stock is low
echo '<span class="crete-remaining-stock red-notice">Only ' . number_format($product->get_stock_quantity(),0,'','') . ' in stock</span>';
} else {
echo '<span class="crete-remaining-stock green-notice">' . number_format($product->get_stock_quantity(),0,'','') . ' in stock</span>';
		}
	}
  }
add_action('crete_woo_inventory_hook', 'crete_woo_inventory_functions');
...
function pivoo_breadcrumb_delimiter( $defaults ) {
 ...
 if( class_exists( 'YITH_WCWL' ) ){
	 ...
	 */
	function crete_product_wishlist_button() {
		?>
		<div class="crete-wishlist-sp">
				<?php echo do_shortcode( '[yith_wcwl_add_to_wishlist]' ); ?>
		
		</div>
		<?php
	}
add_action( 'woocommerce_after_add_to_cart_button', 'crete_product_wishlist_button',10, 0 );
